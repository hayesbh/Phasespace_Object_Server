...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    ROS_WARN("usage: rosrun Phasespace_Object_Server load_env name");
    return 1;
  }
  /*Initialize the service client Node on the add_object service*/
  ros::NodeHandle n;
  ros::ServiceClient client =
    n.serviceClient<Phasespace_Object_Server::load_env>("load_env");
  Phasespace_Object_Server::load_env srv;
